...
class LoginPage extends StatelessWidget {
          ...
          Container(
            ...
            child: Column(
              children: <Widget>[
                ...
                GestureDetector(
                  child: Text(
                    value.isJoin ? 'Dont have an account?' : 'Do you already have an accout?',
                    style: TextStyle(fontWeight: FontWeight.bold),
                  ),
                  onTap: () {
                    value.toggle();
  ...
  void _register(BuildContext context) async {
    ...
    await FirebaseAuth.instance.createUserWithEmailAndPassword(
      ...
      email: _emailController.text, // we need to set a controller with TextFormField widget
    ...
    if (user == null) { // if it didn't work, fulfil this code
      print('fail $user');
      final snackbar = SnackBar(content: Text("Please try again lager.")); // Making a snackbar
      Scaffold.of(context).showSnackBar(snackbar);
    }

    print(user);

    Navigator.push(context, MaterialPageRoute( // Move to another page
        builder: (context) => Timeline(email: user.email)));
  }

  void _login(BuildContext context) async { // Almost same code with _register()
    print("login!!!");
    final AuthResult result =
    await FirebaseAuth.instance.signInWithEmailAndPassword( // Request Sign in with the giving email and password
